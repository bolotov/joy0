...
PUBLIC void inilinebuffer(void);        /* file scan.c		*/
...
PUBLIC void inimem1(void);            /* file utils.c		*/
...
PUBLIC void abortexecution_(void);        /* forward */

PUBLIC void enterglobal(void) {
...
PRIVATE void definition(int hidden) {
    Entry *here = NULL;
...
PRIVATE void defsequence(int hidden) {
    definition(hidden);
    while (sym == SEMICOL) {
        getsym();
        definition(hidden);
