...
PUBLIC void exeterm(Node *n)
{
    ...
    while (conts->u.lis != NULL) {
        ...
        switch (stepper->op) {
            ...
            case STRING_:
            case LIST_:
                NULLARY(stepper->op, stepper->u.num);
                break;
            case USR_:
...
PRIVATE void app2_(void) {            /*   Y  Z  [P]  app2     ==>  Y'  Z'  */
    THREEPARAMS("app2");
    ONEQUOTE("app2");
    SAVESTACK;
...
PRIVATE void treegenrec_(void) {                    /* T [O1] [O2] [C]	*/
    ...
    cons_();
    cons_();
    D(printf("treegenrec: stack = ");
    writeterm(stk);
    printf("\n"); )
    treegenrecaux();
}

PRIVATE void o_online_manual_(void) {
    make_manual(0);
}

PRIVATE void l_latex_manual_(void) {
    make_manual(1);
}
