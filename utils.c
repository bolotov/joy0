...
PUBLIC Node *newnode(Operator o, long l, Node *r) {
    Node * p;
...
PUBLIC void writefactor(Node *n) {
    ...
    switch (n->op) {
        ...
        case LIST_:
            ...
            return;
        case USR_:
            printf("%s", n->u.ent->name);
            return;
        default:
            printf("%s", symtab[(int) n->op].name);
            return;
    }
}
PUBLIC void writeterm(Node *n) {
